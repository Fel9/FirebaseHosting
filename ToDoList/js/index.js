  ...
  firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      ...
      swal
        .fire({
          icon: "success",
          title: "<div style='color: #ddd;'> Redirecionando para a tela de autenticação </div>",
          background: '#2a2a2a',
          focusConfirm: false,
...
function createDelButton(task) {
  ...
  newButton.setAttribute("class", "btn mt-2")
...
function renderTasks() {
  ...
  for (let task of tasks) {
    ...
    newItem.setAttribute(
      "class",
      "list-group-item d-flex justify-content-between align-items-center flex-wrap w-100",
    )
